...
export default class Annotations {
  ...
  updatePosition(camera) {
      ...
      const x = Math.round((0.5 + vector.x / 2) * (self.canvasContainer.offsetWidth / window.devicePixelRatio));
      const y = Math.round((0.5 - vector.y / 2) * (self.canvasContainer.offsetHeight / window.devicePixelRatio));
      const a = document.querySelector(`[data-annotation='${annotation.number}']`);
